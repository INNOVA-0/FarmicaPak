...
weeds_list = ['Chinee apple','Lantana','Negative','Parkinsonia','Parthenium','Prickly acacia','Rubber vine','siam weed','Snake weed']
...
def weed_library(subject):
    ...
    return render_template('Bio_researcher/weeds_library.html',blogs=blogs,classes=weed_classes,current_class=subject,searching=searching)
...
def weed_library_search():
    ...
    return render_template('Bio_researcher/weeds_library.html',blogs=blogs,classes=weed_classes,current_class=class1,searching=searching)
...
def profile(subject):
    ...
    if db.child("Library").child('Weed_classes').child(subject+"").get().val():
        ...
        pass
    return render_template('Bio_researcher/profile.html',blogs=blogs,user_data=values,classes=weed_classes,user_id=key,subject=subject)
...
def update_researcher():
    if 'useremail' in session:
        if request.method == 'POST':
            ...
            db.child('Bio_researcher').child(key1+"").child(key1+"1").update({"username": username,'about':aboutme,'profileimg':myfilename})
...
def add_blog():
    if 'useremail' in session:
        ...
        return render_template('Bio_researcher/add_blog.html',user_data=values,classes=weed_classes,blogs_classes=weed_classes_posted,user_id=key)
...
def classifier():
    if request.method == 'POST':
            ...
            img = cv2.imread(app.config['UPLOAD_FOLDER']+'/'+'image.jpg')
            output1 = cv2.resize(img,(299,299))
            data = np.expand_dims(output1, 0)
            result = model.predict(data)
            output = weeds_list[np.argmax(result)]
            description =" nothing"
            links = "not available"
            ...
            return render_template('classifier/index.html',filename='image.jpg',desc=description,links=links,weedclass=output)
...
def admin_index():
    if 'adminemail' in session:
        ...
        weeds_list = ['Chinee apple','Lantana','Parkinsonia','Parthenium','Prickly acacia','Rubber vine','siam weed','Snake weed']
        count_blog = []
        for i in weeds_list:
        ...
        data = {'Task' : 'Hours per Day', 'Chinee apple' : count_blog[0], 'Lantana' : count_blog[1], 'Parkinsonia' : count_blog[2], 'Parthenium' : count_blog[3], 'Prickly acacia' : count_blog[4],'Rubber vine':count_blog[5],'siam weed':count_blog[6],'Snake weed':count_blog[7]}
        return render_template('admin/index.html',records=list,weed_classes=weeds_classes,admins=admins,data=data,data1=data)
...
def Botanist_list():
    if 'adminemail' in session:
        ...
        v = 0
        for i in zip1:
            Botanists_data.append([keys_botanists[v],list(i[1].val().values())[0]['username'],list(i[1].val().values())[0]['email'],x[v]])
